...
const httpWithCors = corsRouter(http, {
  allowedHeaders: ["Content-Type", "X-Tuttu-Admin-Token"],
});
  ...
  handler: httpActionWithErrorHandling(async (ctx, request) => {
    return new Response(null, {
      ...
      headers: {
        ...
        "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Tuttu-Admin-Token",
...
http.route({
  ...
  handler: httpActionWithErrorHandling(async (ctx, request) => {
    ...
    const header = request.headers.get("X-Tuttu-Admin-Token");
    const authHeader = request.headers.get("Authorization");
    if (authHeader === null) {
      if (header !== process.env.TUTTU_ADMIN_TOKEN) {
