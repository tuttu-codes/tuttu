...
export const createAdminShare = internalMutation({
  ...
  handler: async (ctx, { chatId }) => {
    ...
    const existing = await ctx.db
      ...
      .unique();
    if (existing) {
      console.log(`Already have a share for chat ${chatId}: Go to https://tuttu.show/${existing.code}`);
      return existing._id;
    }
    ...
    console.log(`Created admin share for chat ${chatId}. Go to https://tuttu.show/${code}`);
    return id;
  },
});
