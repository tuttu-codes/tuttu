...
async function isTuttuRunning(url: string): Promise<boolean> {
  try {
...
async function handleSignIn(page: Page, tuttuUrl: string, credentials: { email: string; password: string }) {
  const signInPage = await page.context().newPage();
  await signInPage.goto(`${tuttuUrl}/signin?use-email=1`, {
    timeout: 60000,
  });
...
export async function generateApp({ prompt, tuttuUrl, outputDir, headless = true, credentials }: GenerateAppOptions) {
  if (!credentials.email || !credentials.password) {
    throw new Error('Email and password are required');
  }

  if (!(await isTuttuRunning(tuttuUrl))) {
    throw new Error(
      `No Tuttu server found at ${tuttuUrl}. Please start the appropriate server first:\n` +
  ...
  try {
    ...
    await page.goto(tuttuUrl);
    await handleSignIn(page, tuttuUrl, credentials);
