...
import type { EditorDocument, ScrollPosition } from 'tuttu-agent/types';
import { ActionRunner } from '~/lib/runtime/action-runner';
import type { ActionCallbackData, ArtifactCallbackData } from 'tuttu-agent/message-parser';
...
import type { FileMap } from 'tuttu-agent/types';
import type { AbsolutePath } from 'tuttu-agent/utils/workDir';
import { getAbsolutePath, getRelativePath } from 'tuttu-agent/utils/workDir';
...
import { parsePartId, type PartId, type MessageId } from 'tuttu-agent/partId.js';
import { generateReadmeContent } from '~/lib/download/readmeContent';
import { setupMjsContent } from '~/lib/download/setupMjsContent';
import type { ConvexProject } from 'tuttu-agent/types';
...
export class WorkbenchStore {
  ...
  async downloadZip(args: { convexProject: ConvexProject | null }) {
    ...
    // Add a README.md file specific to Tuttu here, but don't clobber an existing one
    const readmeContent = generateReadmeContent(
      description.value ?? 'project',
      args.convexProject?.deploymentName ?? null,
    );
    const readmePath = hasReadme ? `TUTTU_README.md` : 'README.md';
