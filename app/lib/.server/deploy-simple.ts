...
export async function deploy({ request }: ActionFunctionArgs) {
  try {
    ...
    let tuttuDeploySecret: string | undefined;

    if (globalThis.process.env.TUTTU_DEPLOY_SECRET) {
      tuttuDeploySecret = globalThis.process.env.TUTTU_DEPLOY_SECRET;
    }
    ...
    if (tuttuDeploySecret) {
      headers['convex-tuttu-deploy-secret'] = tuttuDeploySecret;
    }
