...
import { ROLE_SYSTEM_PROMPT, generalSystemPrompt } from 'tuttu-agent/prompts/system';
import { deployTool } from 'tuttu-agent/tools/deploy';
import { viewTool } from 'tuttu-agent/tools/view';
import type { ConvexToolSet } from 'tuttu-agent/types';
import { npmInstallTool } from 'tuttu-agent/tools/npmInstall';
import type { Tracer } from '~/lib/.server/chat';
import { editTool } from 'tuttu-agent/tools/edit';
import { captureException, captureMessage } from '@sentry/remix';
import type { SystemPromptOptions } from 'tuttu-agent/types';
...
import { calculateTuttuTokens, usageFromGeneration } from '~/lib/common/usage';
import { lookupDocsTool } from 'tuttu-agent/tools/lookupDocs';
import { addEnvironmentVariablesTool } from 'tuttu-agent/tools/addEnvironmentVariables';
...
async function storeDebugPrompt(
  ...
  try {
    ...
    const { tuttuTokens } = calculateTuttuTokens(usage, modelProvider);
