...
const ShowInner: FC<ShowInnerProps> = ({ share, className }) => {
  ...
  // Tuttu SVG icon inline
  const TuttuIcon = (props: React.SVGProps<SVGSVGElement>) => (
  ...
  return (
    <div className={classNames('mx-auto flex w-full flex-col gap-2 p-4 md:max-w-3xl min-h-screen', className)}>
      ...
      <div className="mb-1 grid w-full grid-cols-1 items-center gap-2 md:grid-cols-2">
        ...
        {/* Desktop: Clone app in Tuttu with Tuttu icon, mobile: What's Tuttu? */}
        <div className="flex justify-end md:justify-end">
          <Button
            ...
            tip={
              typeof window !== 'undefined' && window.innerWidth < 768
                ? undefined
                : `New users: clone this app and get 85,000 bonus Tuttu tokens, plus some for the author of this app!
` /* TODO this is buggy */
            }
          >
            <span className="hidden md:inline">Clone app in Tuttu</span>
            <span className="inline md:hidden">What&apos;s Tuttu?</span>
          </Button>
        </div>
      </div>
      {/* Second row: author and Try App/Tuttu button, responsive */}
      <div className="mb-1 grid w-full grid-cols-1 items-center gap-2 md:grid-cols-2">
      ...
      {share.referralCode && (
        <div className="mt-2 hidden rounded-lg border border-bolt-elements-background-depth-3 bg-bolt-elements-background-depth-2 p-4 md:block">
          <div className="text-center">
            <p className="text-base font-medium text-content-primary">Join Convex and get 85,000 bonus Tuttu tokens.</p>
            <p className="mt-1 text-xs text-content-secondary">
              When you sign up, {author.username} will earn bonus resources too!
            </p>
            <Button
              href={`https://convex.dev/try-tuttu/${share.referralCode}`}
              target="_blank"
              variant="primary"
              className="mt-2"
            >
