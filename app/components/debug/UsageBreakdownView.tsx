...
type DebugUsageData = {
  ...
  chatTotalTuttuTokens: number;
  chatTotalTuttuBreakdown: TuttuBreakdown;
  ...
  useEffect(() => {
    if (fileContent !== null) {
      async function parseFileContent(blob: Blob) {
        ...
        setTuttuDebugProperty('messages', messages);
      }
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-4">
      ...
      <BreakdownView
        ...
        tuttuTokens={usageData.chatTotalTuttuTokens}
        tuttuBreakdown={usageData.chatTotalTuttuBreakdown}
        title="Total Usage"
        startOpen={true}
      />
      <div>
        ...
        {usageData?.usagePerMessage.map((usage) => (
          <CollapsibleView
            title={`Message ${usage.messageIdx} -- ${formatNumber(usage.tuttuTokens)} tuttu tokens`}
          ...
          >
            <div className="ml-4">
              ...
              <BreakdownView
                ...
                tuttuTokens={usage.tuttuTokens}
                tuttuBreakdown={usage.tuttuBreakdown}
              />
                ...
                <div className="ml-4 flex flex-col gap-4">
                  {usage.parts.map((part, idx) => (
                    <div className="flex flex-col gap-4" key={idx}>
                      <CollapsibleView
                        title={`Part ${idx} -- Tuttu Tokens: ${formatNumber(part.usageInfo?.tuttuTokens ?? 0)}`}
                        startOpen={false}
                      >
                        <div className="ml-4 flex flex-col gap-4">
                          ...
                          {part.usageInfo ? (
                            <BreakdownView
                              ...
                              tuttuTokens={part.usageInfo.tuttuTokens}
                              tuttuBreakdown={part.usageInfo.tuttuBreakdown}
                            />
                          ) : (
                            <p>No usage info</p>
                          )}
...
function BreakdownView({
  ...
  const tokensData = {
    'Prompt - Anthropic (Uncached)': tuttuBreakdown.promptTokens.anthropic.uncached,
    'Prompt - Anthropic (Cached)': tuttuBreakdown.promptTokens.anthropic.cached,
    'Prompt - OpenAI (Uncached)': tuttuBreakdown.promptTokens.openai.uncached,
    'Prompt - OpenAI (Cached)': tuttuBreakdown.promptTokens.openai.cached,
    'Prompt - XAI (Uncached)': tuttuBreakdown.promptTokens.xai.uncached,
    'Prompt - XAI (Cached)': tuttuBreakdown.promptTokens.xai.cached,
    'Prompt - Google (Uncached)': tuttuBreakdown.promptTokens.google.uncached,
    'Prompt - Google (Cached)': tuttuBreakdown.promptTokens.google.cached,
    'Completion - Anthropic': tuttuBreakdown.completionTokens.anthropic,
    'Completion - OpenAI': tuttuBreakdown.completionTokens.openai,
    'Completion - XAI': tuttuBreakdown.completionTokens.xai,
    'Completion - Google': tuttuBreakdown.completionTokens.google,
  };
  return (
    <div>
      {title && <h4>{title}</h4>}
      <div>
        ...
        <CollapsibleView title="Tuttu Breakdown" startOpen={true}>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-4">
              <p className="text-2xl font-bold">{formatNumber(tuttuTokens)}</p>
              <div className="flex flex-wrap gap-8">{renderPieChart(tokensData)}</div>
            </div>
            <div>
              <JsonView data={tuttuBreakdown} shouldExpandNode={(level) => level < 1} />
            </div>
          </div>
        </CollapsibleView>
      </div>
...
async function getUsageBreakdown(messages: Message[]) {
  ...
  let chatTotalTuttuTokens = 0;
  const chatTotalTuttuBreakdown: TuttuBreakdown = {
    completionTokens: {
      anthropic: 0,
      openai: 0,
      xai: 0,
  ...
  for (const [idx, message] of messages.entries()) {
    ...
    const { tuttuTokens, breakdown } = calculateTuttuTokens(totalUsageBilledFor, provider);
    ...
    addBreakdown(chatTotalTuttuBreakdown, breakdown);
    chatTotalTuttuTokens += tuttuTokens;
  }
  return {
    chatTotalRawUsage,
    chatTotalUsageBilledFor,
    chatTotalTuttuTokens,
    chatTotalTuttuBreakdown,
    usagePerMessage,
  };
}
...
function getPartInfos({
  ...
  for (const [idx, part] of message.parts?.entries() ?? []) {
    if (part.type === 'text') {
      ...
      const { tuttuTokens, breakdown } = calculateTuttuTokens(billedUsageForPart, provider);
      partInfos.push({
        partIdx: idx,
  ...
  const { tuttuTokens, breakdown } = calculateTuttuTokens(finalUsage, provider);
  partInfos.push({
...
function addBreakdown(breakdownA: TuttuBreakdown, update: TuttuBreakdown) {
