...
function LittleUsage({
  ...
  const detailedLabel = isPaidPlan
    ? `${displayTuttuTokenNumber(used)} tokens used / ${displayTuttuTokenNumber(quota)} included (${Math.floor(usagePercentage)}%)`
    : isLoadingUsage
      ? ''
      : usagePercentage < 100
        ? `${displayTuttuTokenNumber(used)} tokens used / ${displayTuttuTokenNumber(quota)} (${Math.floor(usagePercentage)}%)`
        : usingApiKey
          ? `Out of tokens (${displayTuttuTokenNumber(used)} used), using API key`
          : `Out of tokens`;
        ...
        {loading ? null : (
          <div>
            ...
            <p className="mt-1 text-xs text-content-secondary">
              {isPaidPlan
                ? `Tuttu tokens power code generation. Your team's Tuttu tokens reset to ${displayTuttuTokenNumber(quota)} on your regular billing cycle. Unused tokens from the previous month are not carried over. Additional Tuttu tokens cost $10 per 1M tokens.`
                : 'Tuttu tokens power code generation. Tokens reset on the first of each month and tokens from the previous month are not carried over.'}
            </p>
            <ul className="mt-2 space-y-2 text-sm text-content-primary">
              {isPaidPlan ? null : (
                <li className="mt-2 border-t pt-2">
                  <Button
                    href={`https://dashboard.convex.dev/t/${teamSlug}/settings/billing?source=tuttu`}
                    target="_blank"
                    variant="unstyled"
                ...
                <li className="border-t pt-2">
                  <div className="flex flex-col items-center gap-2">
                    <p>
                      ...
                      to get 85K additional Tuttu tokens per month.
                    </p>
              ...
              <li className="mt-2 border-t pt-2 text-xs text-content-secondary">
                {usingApiKey ? (
                  usagePercentage >= 100 ? (
                    "You're using an API key so can keep building without using Tuttu tokens."
                  ) : (
                    "You have an API key set for the model you're using so you'll be able to keep building after running out of Tuttu tokens."
                  )
                ) : (
                  <>
                    <Button
                    ...
                    in settings to avoid spending Tuttu tokens.
                  </>
                )}
              </li>
            </ul>
