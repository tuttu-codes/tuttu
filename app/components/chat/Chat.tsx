...
export const Chat = memo(
  ({
    ...
    setTuttuDebugProperty('parsedMessages', parsedMessages);
    ...
    const sendMessage = async (messageInput: string) => {
      ...
      if (
        ...
        let message: string | ReactNode = 'Tuttu is too busy cooking right now. ';
        if (retries.numFailures >= MAX_RETRIES) {
          message = (
            <>
              {message}
              Please{' '}
              <a href="https://tuttu.convex.dev/settings" className="text-content-link hover:underline">
        ...
        toast.error(message);
        captureMessage('User tried to send message but tuttu is too busy');
        return;
      }
    ...
    <div className="flex w-full flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          href={
            selectedTeamSlug
              ? `https://dashboard.convex.dev/t/${selectedTeamSlug}/settings/billing?source=tuttu`
              : 'https://dashboard.convex.dev/team/settings/billing?source=tuttu'
          }
          className="w-fit"
        ...
        {referralCode && referralStats?.left !== 0 && (
          <div className="w-full space-y-2">
            <p className="text-sm text-content-secondary">
              Refer a friend and Get 85,000 free Tuttu tokens for each
