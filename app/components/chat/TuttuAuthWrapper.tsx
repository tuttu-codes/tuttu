...
const TuttuAuthContext = createContext<{
  state: TuttuAuthState;
}>(null as unknown as { state: TuttuAuthState });

export function useTuttuAuth() {
  const state = useContext(TuttuAuthContext);
  if (state === null) {
    throw new Error('useTuttuAuth must be used within a TuttuAuthProvider');
  }
  return state.state;
}

export function useTuttuAuthContext() {
  const state = useContext(TuttuAuthContext);
  if (state === null) {
    throw new Error('useTuttuAuth must be used within a TuttuAuthProvider');
  }
...
export const TuttuAuthProvider = ({
  children,
  ...
  useEffect(() => {
    function setSessionId(sessionId: Id<'sessions'> | null) {
      ...
      if (sessionId) {
        setTuttuDebugProperty('sessionId', sessionId);
      }
    }
  ...
  const state: TuttuAuthState = isLoading
    ? { kind: 'loading' }
    : isUnauthenticated
      ? { kind: 'unauthenticated' }
  ...
  return <TuttuAuthContext.Provider value={{ state }}>{children}</TuttuAuthContext.Provider>;
};
