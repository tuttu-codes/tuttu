...
import { TuttuResult, TuttuModel } from './types';
import { copyFileSync, mkdirSync, readFileSync, statSync, writeFileSync } from 'fs';
import { execFileSync } from 'child_process';
import { ChatContextManager } from 'tuttu-agent/ChatContextManager';
import { UIMessage } from 'ai';
import { deploy, npmInstall, runTypecheck } from './convexBackend';
import { StreamingMessageParser } from 'tuttu-agent/message-parser';
import { withConvexBackend } from './convexBackend';
import { initializeConvexAuth } from 'tuttu-agent/convexAuth';
import { deployTool } from 'tuttu-agent/tools/deploy';
import { ROLE_SYSTEM_PROMPT } from 'tuttu-agent/prompts/system';
import { generateId } from 'ai';
import { ConvexToolSet, SystemPromptOptions } from 'tuttu-agent/types';
import { npmInstallTool, npmInstallToolParameters } from 'tuttu-agent/tools/npmInstall';
import { lookupDocsTool } from 'tuttu-agent/tools/lookupDocs';
import { getConvexDeploymentNameTool } from 'tuttu-agent/tools/getConvexDeploymentName';
import { cleanupAssistantMessages } from 'tuttu-agent/cleanupAssistantMessages';
import { generalSystemPrompt } from 'tuttu-agent/prompts/system';
import { makePartId } from 'tuttu-agent/partId';
import { logger } from 'tuttu-agent/utils/logger';
import { traced, wrapTraced } from 'braintrust';
import { viewTool } from 'tuttu-agent/tools/view';
import { editTool, editToolParameters } from 'tuttu-agent/tools/edit';
import { renderFile } from 'tuttu-agent/utils/renderFile';
import { renderDirectory } from 'tuttu-agent/utils/renderDirectory';
import { viewParameters } from 'tuttu-agent/tools/view';
import { lookupDocsParameters, docs, type DocKey } from 'tuttu-agent/tools/lookupDocs';
...
export async function tuttuTask(model: TuttuModel, outputDir: string, userMessage: string): Promise<TuttuResult> {
  ...
  const { numDeploys, usage, success } = await withConvexBackend(backendDir, async (backend) => {
    ...
    const opts: SystemPromptOptions = {
      ...
      // TODO: We need to set up a Convex deployment running the `tuttu`
...
async function invokeGenerateText(model: TuttuModel, opts: SystemPromptOptions, context: UIMessage[]) {
  return traced(
    async (span) => {
